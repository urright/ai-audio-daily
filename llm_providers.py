    ...
    def summarize(self, title: str, content: str, prompt_template: str) -> str:
        ...
        if text:
            ...
            try:
                ...
                # 尝试提取文本中的 JSON 块
                json_match = re.search(r'\{[^}]*"summary"\s*:\s*"[^"]*"[^}]*\}', text, re.DOTALL) or \
                             re.search(r'\{[^}]*summary[^}]*\}', text, re.DOTALL)
                if json_match:
                    try:
                        ...
                        pass
            # 不是 JSON，直接返回
            if 10 < len(text.strip()) < 300:
                return text.strip()
        # 从 reasoning 提取
        try:
            reasoning = message.model_dump().get('reasoning') or ''
            if reasoning:
                import json, re
                # 同样尝试 JSON
                json_match = re.search(r'\{[^}]*"summary"\s*:.*?\}', reasoning, re.DOTALL) or \
                             re.search(r'\{[^}]*summary[^}]*\}', reasoning, re.DOTALL)
                if json_match:
                    try:
                        data = json.loads(json_match.group())
                        if data.get("summary"):
